    ...
    <Container className="help">
      ...
      <Row className="indices">
        ...
        <p className="foreword">
          In defining the following indices of temperature, let tas_ij be the
          daily mean temperature on day i in season j; the daily minimum and
          maximum temperature on day i in season j are denoted by tmin_ij and
          tmax_ij respectively.
        </p>
        <p className="contents">
          <b>mean daily near-surface air temperature</b> (tas):{" "}
        </p>
        <p className="contents">
          <b>number of cold spell days</b> (csdi): Count the number of days in
          season j where tas_ij {"<"} 2°C for more than two consecutive days.
        </p>{" "}
        <p className="contents">
          <b>number of heatwave days</b> (hwdi): Count the number of days in
          season j where tmax_ij {">"} t_1 and tmin_ij {">"} t_2 for two or more
          consecutive days, where t_1 and t_2 are thresholds defined for each
          region as in the table below. These thresholds are taken from the{" "}
        ...
        <p className="contents">
          ...
          temperatures tas during the season of interest for all years in the
          selected time period are pooled, and the 1st percentile of the pooled
          seasonal temperatures is calculated.
        </p>{" "}
        <p className="contents">
          ...
          series over the period of interest. Instead, the daily mean
          temperatures tas during the season of interest for all years in the
          selected time period are pooled, and the 99th percentile of the pooled
          seasonal temperatures is calculated.{" "}
        </p>{" "}
        <p className="contents">
          <b>number of cold days</b> (tmax10p): Let q_{10}(tmax_ir) be the c10th
          percentile of daily maximum temperatures in a five-day window centred
          on calendar day i in the reference period r (1980-2010). Count the
          number of days where tmax_ij {"<"} q_{10}(tmax_ir) for the season of
          interest j.{" "}
        </p>{" "}
        <p className="contents">
          <b>number of hot days</b> (tmax90p): Let q_{90}(tmax_ir) be the 90th
          percentile of daily maximum temperatures in a five-day window centred
          on calendar day i in the reference period r (1980-2010). Count the
          number of days where tmax_ij {">"} q_{90}(tmax_ir) for the season of
          interest j.
        </p>{" "}
        <p className="contents">
          ...
          centred on calendar day i in the reference period r (1980-2010). Count
          the number of days where tmin_ij {"<"} q_{10}(tmin_ir) for the season
          of interest j.
        </p>{" "}
        <p className="contents">
          <b>number of warm nights</b> (tmin90p): Let q_{90}(tmin[ir]) be the
          90th percentile of daily minimum temperatures in a five-day window
          centred on calendar day i in the reference period r (1980-2010). Count
          the number of days where tmin_ij {">"} q_{90}(tmin_ir) for the season
          of interest j.
        </p>
      </Row>
      {/* precipitation */}
      <Row className="indices">
        ...
        <p className="foreword">
          In defining the following indices of precipitation, let pr_ij be the
          daily precipitation amount in mm on day i in period j, and let prc_ij
          be the daily precipitation amount arising from convective processes in
          mm on day i in period j. pr_ij geq 1 denotes the subset of the pr_ij
          for which pr_ij geq 1.
        </p>
        ...
        <p className="contents">
          <b>proportion of rainfall due to convection</b> (prcprop): Let prc_ij
          be the daily precipitation amount due to convective rainfall on day i
          in period j. The proportion of total precipitation due to convective
          processes in period j is the sum of prc_ij geq 1 in period j divided
          by the total precipitation in period j, prcptot.
        </p>
        <p className="contents">
          <b>proportion of wet days followed by a wet day</b> (pww): Where n is
          the total number of days in period j, and pr_ij {">"} 1 is an
        ...
        <p className="contents">
          <b>proportion of dry days followed by a dry day</b> (pdd): Where n is
          the total number of days in period j, and pr_ij {">"} 1 is an
          indicator function with value one if pr_ij {">"} 1, and zero
        ...
        <p className="contents">
          ...
          total precipitation amount for the five-day interval ending on day i
          in season j. The maximum five-day values for period j are max(pr5_ij).
        </p>
        <p className="contents">
          <b>
          ...
          (r95ptot): Let q_{95}(pr_r) be the 95th percentile of daily wet-day
          precipitation in the reference period r (1980-2010). The total
          precipitation exceeding the 95th percentile is the sum of pr_ij geq q_
          {95}(pr_r).
        </p>
        <p className="contents">
          <b>
          ...
          (r99ptot): Let q_{99}(pr_r) be the 99th percentile of daily wet-day
          precipitation in the reference period r (1980-2010). The total
          precipitation exceeding the 99th percentile is the sum of pr_ij geq q_
          {99}(pr_r).
        </p>
        <p className="contents">
        ...
        <p className="contents">
          ...
          consecutive days where pr_ij {"<"} 1.
        </p>
        <p className="contents">
          <b>maximum length of dry spell</b> (dsmax): Count the largest number
          of consecutive days where pr_ij {"<"} 1.
        </p>
        <p className="contents">
          <b>median length of wet spell</b> (wsmed): Count the median number of
          consecutive days where pr_ij geq 1.
        </p>
        <p className="contents">
          <b>maximum length of wet spell</b> (wsmax): Count the largest number
      ...
      <Row className="indices">
        <div>
          ...
          <p className="contents">
            ...
            season j is the count of days where swind geq 10.8ms<sup>-1</sup>-.
          </p>
        </div>
      </Row>
      {/* extreme events */}
      <Row className="indices">
        <div>
          ...
          <p className="foreword">
            ...
            season and then averaged over the selected time period. Instead, let
            max(idx_j) be the maximum daily value of some index idx_ij in each
            season j, and let max(idx_j) denote the set of all such maxima
            during the time period of interest: this set contains 30 seasonal
            maxima for future periods, and 20 maxima for the evaluation period.
            A generalised extreme value distribution citep[GEV:][]Coles2001 is
            fitted to the seasonal maxima max(idx_j) at each grid cell, and the
            parameters of this distribution are used to calculate selected
            return periods (the expected number of years between occurrences of
            events of a given magnitude) and return levels (the magnitude of a
            once-in-x-years event).<br></br> Indices of extreme events are
            available for precipitation indices rx1day and rx5day, and for wind
            indices sfcWindmax and wsgsmax. For any of these indices, denoted
            here by the generic index 'idx', the following plots are available:
          </p>
          <p className="contents">
        ...
        <div>
          ...
          <p className="contents">
            ...
            <Latex displayMode={true}>
              ...
              and R the relative humidity hursij for day i in period j. Then the
              heat index for day i in period j is HI_ij = c1 + c2T + c3R + c4TR
              + c5T2 + c6R2 + c7T2R + c8TR2 + c9T2R2 where the ci are constants
              as given in equation 1 of ?. The heat index is only valid for air
              temperatures above 20◦C and will be reported as NA when T ≤20◦C.
              The number of days in period j on which the heat index is
              dangerously high is given by the count of HIij ≥32◦C.
            </Latex>{" "}
          </p>
          <p className="contents">
          ...
          <p className="contents">
            <b>potential evapotranspiration</b> (pet): PET is computed for each
            day i in season j using the REC: McGuinness-Bordne equation, where λ
            is a constant representing the latent heat of vaporisation and S(i)
            0 is the extraterrestrial radiation on day i, estimated from the
            time of year and latitude. Seasonal PET time series are computed by
          ...
          <p className="contents">
            ...
            is computed in the same way as spei6, but using a six-month rolling
            average of prcptot rather than the climatic water balance.
          </p>
          <p className="contents">
            ...
            precipitation prcptot, and computing a six-month rolling average.
            This six-month average water balance is fitted to a log-logistic
            distribution in order to transform the original values to
            standardised units. Seasonal time series of spei6 are computed by
            averaging the monthly values within each season.
          </p>
          <p className="contents">
      ...
      <Row className="indices">
        <div>
          ...
          <p className="foreword">
            ...
            degree-days are based on calendar years, rather than meteorological
            years.
          </p>
          <p className="contents">
